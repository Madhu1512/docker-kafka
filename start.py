...
def main(loops=-1, loop_interval=60, restart_interval=30):
    ...
    kafka_pid = start_kafka(base_properties, zk_conn, exhibitor)
        ...
        if cur_zk != zk_conn and len(cur_zk) >= len(zk_conn):
            ...
            try:
                ...
                with zk.Lock('/kafka-exhibitor/%s' % exhibitor):
                    ...
                    kafka_pid.terminate()
                    kafka_pid.wait()
                    kafka_pid = start_kafka(base_properties, cur_zk, exhibitor)
                    zk_conn = cur_zk
...
def start_kafka(base_properties, zk_conn, exhibitor):
    kafka_con = kafka_conn_string(exhibitor)
    properties = base_properties.replace('{{KAFKA_BROKER_ID}}', kafka_con).replace('{{ZOOKEEPER_CONNECTION_STRING}}',
                                         ','.join(zk_conn))
...
def kafka_conn_string(exhibitor):
    zk_conn = zk_conn_string(exhibitor)
    zk = KazooClient(hosts=','.join(zk_conn))
    try:
        zk.start()
    except:
        logger.info("no ZooKeeper server found")
        raise
    zk.start()
    broker_id = ''
    try:
